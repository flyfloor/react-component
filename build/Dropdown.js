...
DropDown.Option = React.createClass({
    ...
    render() {
        let node = this.props.selected ? React.createElement(
            'i',
            null,
            '√'
        ) : null;
...
DropDown.multiInput = React.createClass({
    ...
    render() {
        const TAGS = this.props.selectedTags.map((tag, index) => {
            return React.createElement(
                'span',
                { className: '_tag', key: index, onClick: this.removeSelected },
                React.createElement(
                    'san',
                    { className: '_text' },
                    tag
        ...
        let placeHolder = this.props.selectedTags.length === 0 && !this.state.hasInput ? React.createElement(
            'span',
